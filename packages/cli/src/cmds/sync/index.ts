...
export function registerSync(program: Command) {
  program
    ...
    .option("--clean", "Run @lytos clean --force before installing")
...
`)
    .action(async (opts: {
      ...
      try {
        ...
        // 4) Clean (optional)
        if (opts.clean) {
          console.log("➡️  @lytos clean --force");
          try {
            sh("@lytos clean --force");
          } catch {
        ...
        console.error("❌ @lytos sync failed:", e instanceof Error ? e.message : e);
        process.exit(1);
      }
    });
}
