...
#   ./scripts/install-cli.sh --uninstall     # npm rm -g @lytos/cli
...
PKG_NAME="@lytos/cli"   # cambia si tu package tiene otro nombre
...
if [ "$MODE" = "link" ] || [ "$DO_LINK" = "true" ]; then
  ...
  if confirm "Proceed with 'npm link' inside $CLI_PKG_DIR?"; then
    ...
    say "✅ Linked globally. Prueba:  @lytos/cli --help   (o: @lytos/cli --help, @lytos/cli --help)"
  else
    warn "Aborted."
  fi
  exit 0
...
if confirm "Install $ABS_TARBALL globally via npm?"; then
  ...
  say "✅ Installed globally. try:  @lytos/cli --help"
  rm -f "$ABS_TARBALL"
