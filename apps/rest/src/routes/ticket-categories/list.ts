import { authorizePermission, getAllTicketCategories } from "@lytos/business-logic";
...
export const listTicketCategoriesRoute = makeFastifyRoute(
    RouteMethod.GET,
    "/",
    verifyJwt,
    ...
    async (req, reply) => {
        ...
        await authorizePermission({ workspaceId: wsId, roleId, permission: "ticket-categories.read" });

        const out = await getAllTicketCategories({ ...params, workspaceId: wsId });
