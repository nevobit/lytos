...
import { authorizePermission, createTicketStatus } from "@lytos/business-logic";
import type { CreateTicketStatusDto } from "@lytos/contracts";

export const createTicketStatusRoute = makeFastifyRoute(
    RouteMethod.POST,
    "/",
    verifyJwt,
    ...
    async (req, reply) => {
        ...
        try {
            const dto = req.body as CreateTicketStatusDto;
            const created = await createTicketStatus(workspaceId, dto);
