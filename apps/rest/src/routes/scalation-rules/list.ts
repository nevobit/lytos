import { authorizePermission, getAllScalationRules } from "@lytos/business-logic";
...
export const listScalationRulesRoute = makeFastifyRoute(
    RouteMethod.GET,
    "/",
    verifyJwt,
    ...
    async (req, reply) => {
        ...
        await authorizePermission({ workspaceId: wsId, roleId, permission: "scalation-rules.read" });

        const out = await getAllScalationRules({ ...params, workspaceId: wsId });
