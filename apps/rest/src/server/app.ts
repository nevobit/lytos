...
export const buildApp = (opts: BuildAppOpts) => {
    ...
    app.addHook("onRequest", async (req) => {
        await registerTenancy(req, {
            baseDomain: process.env.BASE_DOMAIN ?? "lytos.app",
            pathPrefix: "/w",
            apiPrefix: "/api/v1",
            allowPathMode: true,
            allowHeaderMode: true,
            allowHostMode: true,
            tenantHeaderName: "x-workspace",
            reservedSubdomains: ["www", "api", "admin", "docs"],
        });
    ...
    app.decorate("requireTenant", () => {
        return async (req: FastifyRequest) => {
            if (!req.tenant) throw new Error("Tenant is required");
        };
    });
