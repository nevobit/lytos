...
- Larger @lytos size; requires discipline with boundaries.
...
- Requires a custom CLI command (`@lytos install`) to enforce usage.
...
**Decision**  
Provide an **internal CLI** (`@lytos` or `@lytos`) under `packages/cli` that wraps common commands:
